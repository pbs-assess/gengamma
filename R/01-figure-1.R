...
ggplot(data = fig1_dat, aes(x = x, y = density, colour = family)) +
  ...
  geom_line(data = filter(fig1_dat, family != "gengamma"), aes(linetype = family), linewidth = 1.5, alpha = 0.6) +
  geom_line(data = filter(fig1_dat, family == "gengamma"), aes(linetype = family), linewidth = 1.5) +
