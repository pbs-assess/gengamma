...
family_colours <- c('tweedie' = "#2271B2", #'honolulu blue'
                    'delta-lognormal' = "#359B73", #'ocean green'
                    'delta-gamma' = "#d55e00", # 'bamboo'
                    'delta-gengamma' = "#F748A5") # 'barbie pink'
...
aic_plot <-
  ggplot() +
    geom_tile(data = aic_plot_df |> distinct(species, region, .keep_all = TRUE),
      mapping = aes(x = 1, y = fspecies2,
      width = Inf, height = 1, fill = factor(odd_species2))) +
    scale_fill_manual(values = c("grey95", "white")) +
    scale_x_continuous(trans = "log10", labels = scales::label_number(trim = TRUE),
      limits = c(0.04, 1100)) +
    gfplot::theme_pbs(base_size = 12) +
    facet_wrap(~ region, ncol = 3, drop = FALSE) +
    geom_point(data = aic_plot_df |> filter(converged == 1, prop_pos >= 0.05),
              aes(x = x, y = species, colour = family),
              stroke = 0.5, size = 2, position = ggstance::position_dodgev(height = 0.5)) +
    geom_point(data = aic_plot_df |> filter(converged == 0, prop_pos >= 0.05),
              aes(x = x, y = species, colour = family),
              shape = 21, stroke = 0.7, size = 2) +
    scale_colour_manual(values = family_colours) +
    labs(x = "Delta AIC + 1", y = "Species") +
    guides(fill = "none") +
    geom_text(data = aic_plot_df |>
      distinct(species, region, .keep_all = TRUE) |>
      filter(prop_pos >= 0.05),
              aes(x = 0.08, y = fspecies2,
              label = round(mean_pos)), #paste0(round(mean_pos), "(", prop_pos, ")")),
              hjust = 1, colour = "grey50",
              size = 2.5) +
    geom_text(data = mean_sets, aes(x = 0.08, y = length(unique(aic_plot_df$species)),
      label = mean_sets), vjust = -2, hjust = 1, size = 3) +
    # geom_text(data = aic_plot_df |> filter(prop_pos >= 0.05), aes(x = 0.04, y = fspecies2, label = signif(est_q, 2)),
    #           size = 2.5, hjust = 0) +
    coord_cartesian(clip = "off")
aic_plot
ggsave(aic_plot, width = 16, height = 11, filename = file.path(fig_dir, 'aic-plot-spp-code-order.png'))
